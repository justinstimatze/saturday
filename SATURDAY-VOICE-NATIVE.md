...
- **Whether Instructions for Claude and Preferences actually behave differently in voice mode at all.** Earlier in testing, Preferences appeared to work where Instructions for Claude didn't — but that comparison wasn't a controlled test (different wording, possibly confounded with the STT recognition problem above, possibly a propagation-timing artifact) and a later test showed Instructions for Claude working fine. Don't treat the field choice as load-bearing until re-tested cleanly.
...
- **Session-name inventory pushed back to Drive — write half built, read half unverified.** `saturday-backend` now writes a small `saturday-sessions.txt` manifest into the same folder every poll tick, listing every live session by project name (`manifest.go`). This closes a real gap found live 2026-08-24: voice mode had no way to know which session names actually exist, and a stale test note ("Lucida," no live session by that name) got force-routed onto whatever session happened to be live instead of being caught. What's *not* yet confirmed: whether voice mode reliably checks that file before naming a session in a note — that needs an account-wide instruction telling it to, which is a change to the user's own Claude account Preferences/Instructions (not something in this repo), and needs the same kind of live hands-on test the write side already got, not just an assumption that "the file exists" implies "voice mode reads it."
