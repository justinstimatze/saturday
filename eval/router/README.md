...
The 0.95-on-wrong is the dangerous mode for silent routing. **Mediator gate
(TRIZ #24) at the routing step catches this**: the gate proposes a target out
loud, the user has ~1s to interrupt, silent inject only on no-interrupt. With
the gate, 93% raw routing accuracy is comfortably shippable.
