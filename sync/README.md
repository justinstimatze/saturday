...
```bash
cd sync && go build -o ~/go/bin/saturday-sync .
```

## Install

```bash
jq '.hooks.UserPromptSubmit += [{"matcher":"","hooks":[{"type":"command","command":"/home/<user>/go/bin/saturday-sync"}]}]' ~/.claude/settings.json > /tmp/s.new && mv /tmp/s.new ~/.claude/settings.json
```
...
these handle all observed `/compact`-related noise. See
`/home/gas6amus/.claude/projects/.../memory/feedback_no_friction_theater.md`
for the philosophy on filtering vs gating.
