...
//
// Wired in ~/.claude/settings.json — CC's hook runner needs an absolute
// path; substitute the output of `go env GOPATH`:
//
//	"UserPromptSubmit": [{ "hooks": [{ "type": "command",
//	   "command": "<GOPATH>/bin/saturday-hook prompt" }] }]
//	"Stop": [{ "hooks": [{ "type": "command",
//	   "command": "<GOPATH>/bin/saturday-hook stop" }] }]
//
...
// Local `go build` keeps the "dev" placeholder.
var version = "dev"
