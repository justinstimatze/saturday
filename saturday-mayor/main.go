...
// Per user 2026-05-03: drop "injecting" — project name + braille bar is
// enough signal.
...
// lastLeafUUID returns the leafUuid from the most recent "last-prompt"
// entry in the JSONL — CC's pointer to the conversation-tree leaf.
// Used as parentUuid for our direct-written user turn so the chain stays
// intact. Returns empty string if no last-prompt entry exists yet (fresh
// session); caller writes parentUuid=null in that case.
func lastLeafUUID(jsonlPath string) (string, error) {
	f, err := os.Open(jsonlPath)
	if err != nil {
		return "", err
	}
	...
	for sc.Scan() {
		...
		if t.Type == "last-prompt" && t.LeafUUID != "" {
			leaf = t.LeafUUID
		}
	}
	return leaf, sc.Err()
...
//
// Schema discovered empirically against CC 2.1.126; if CC changes its
// JSONL format, this writes get rejected on next resume — an INJECTION.md
// gotcha worth pinning. flock-protected against concurrent writers.
func directWriteUserTurn(jsonlPath, sessionID, cwd, text string) error {
	...
	leaf, err := lastLeafUUID(jsonlPath)
	if err != nil {
