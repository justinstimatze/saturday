...
```bash
make install        # → $(go env GOPATH)/bin/saturday-mayor (and the others)
```
...
3. `<scriptDir>/.env` — falls back to legacy in-repo location for dev.
   When mayor is installed under the Go bin directory (no `main.go` in
   that dir), `scriptDir` resolves to the current working directory.
...
```bash
saturday-watcher &              # in one terminal
saturday-mayor                  # in another
saturday-mayor --dry-run        # log proposals, don't inject
```
