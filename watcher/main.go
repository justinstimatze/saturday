...
func decodeProjectName(encodedDir string) string {
	...
	if !strings.HasPrefix(encodedDir, prefix) {
		return encodedDir
	}
	trailing := encodedDir[len(prefix):]
...
func newWatcher(projectsDir string, activeWindow time.Duration, initialReplay int) *Watcher {
	return &Watcher{
...
func (w *Watcher) scan(initial bool) {
	...
	for _, projDir := range entries {
		...
		latest := mostRecentJSONL(filepath.Join(w.projectsDir, projDir.Name()))
		if latest == "" {
			continue
		}
		...
		w.processFile(latest, projDir.Name(), info, initial)
	}
}
...
func (w *Watcher) processFile(path, encodedDir string, info os.FileInfo, initial bool) {
	w.mu.Lock()
	entry, exists := w.sessions[path]
	if !exists {
		entry = &SessionEntry{
			State:     State{Project: decodeProjectName(encodedDir)},
			JSONLPath: path,
		}
		if initial {
...
func main() {
	...
	root := flag.String("root", "", "Claude Code projects dir (default ~/.claude/projects)")
	...
	log.Printf("saturday-watcher — root=%s interval=%s active=%s replay=%d",
		projectsDir, *interval, *activeWindow, *initialReplay)

	w := newWatcher(projectsDir, *activeWindow, *initialReplay)
