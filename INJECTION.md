...
Ran `/tmp/saturday-probe/probe.sh` to verify behavior. Findings:

- **Phase 1 (cold session creation, fresh sid):**
  exit=0, 9.4s wall, 1.3s API, $0.10948. The 16k input tokens are mostly
  CLAUDE.md auto-load — sunk cost per fresh session, not per inject.

- **Phase 2 (resume + print):**
  exit=0, 1.3s wall, $0.019. 32k cache-read tokens, 446 cache-creation.
  Model correctly recalled the "42" from phase 1 across processes. Confirms
  `--resume` reconstructs the conversation from JSONL and the API has full
  prior context.
...
Keeps the 1h cache hot indefinitely. Saturday's drivermap or active-window
filter decides which sessions qualify — wasteful to ping all 6-8 sessions
when 3-4 are active.
...
drivermap currently considers active, never automated against sessions
...
| component         | what it is                                       | LoC      |
|-------------------|--------------------------------------------------|----------|
| injector          | one-liner: `claude --resume <sid> --print '<text>'` | 0        |
| sync hook         | UserPromptSubmit JSONL-cursor diff + note inject | ~300 Go  |
| saturday-mayor    | orchestrates watcher → router → expander → inject | ~490 Go  |

The keystroke-injection binary that V0.1 originally listed dissolves to
zero LoC. The sync hook is the new V0.1 component the headless approach
introduces.
